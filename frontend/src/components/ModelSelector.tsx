  ...
  if (!modelsLoading && models.length === 0) {
    return (
      <button
        ...
        className="flex items-center gap-2 w-full px-2 py-1.5 rounded-lg border border-dashed border-gray-300 dark:border-zinc-700 text-gray-400 dark:text-zinc-500 hover:border-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-colors"
      >
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <select
        ...
        className="flex-1 bg-white border border-gray-300 dark:bg-zinc-800 dark:border-zinc-700 rounded-lg px-2 py-1.5 text-xs text-gray-700 dark:text-zinc-300 focus:outline-none focus:border-indigo-500 cursor-pointer"
      >
        {models.map((m) => (
